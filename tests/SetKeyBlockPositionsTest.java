...
public class SetKeyBlockPositionsTest {
     ...
     */
    final private int BLOCK_LENGTH = StepperAppFields.BLOCK_LENGTH;

    /**
     * Equal to `StepperAppFields.BLOCK_COUNT`. Shortened for extra readability. (Static imports didn't work)
     */
    final private int BLOCK_COUNT = StepperAppFields.BLOCK_COUNT;
    ...
    private void printAssert(byte[] expected, byte[] result) {
        ...
        if(BLOCK_LENGTH != StepperAppFields.BLOCK_LENGTH) {
        ...
        if(BLOCK_COUNT != StepperAppFields.BLOCK_COUNT) {
